// ============================================
// RN SPORTS HUB — Cart v1
// ============================================

const CART_KEY = "rn_sports_cart_v1";
...
window.cartAdd = function(product, size, qty = 1) {
  ...
  if (existing) { existing.qty += qty; }
    ...
    cartItems.push({
      key, productId: product.id, name: product.name, price: product.price,
...
window.cartUpdateQty = function(key, delta) {
  ...
  if (!item) return;
  item.qty += delta;
  if (item.qty <= 0) { window.cartRemove(key); }
  else { saveCart(); cartRefreshUI(); renderCartItems(); }
};

window.cartClear = function() { cartItems = []; saveCart(); cartRefreshUI(); };
window.loadCart = function() { cartItems = loadCartFromStorage(); return cartItems; };
window.getCartTotal = function() { return cartItems.reduce((s,i) => s + i.price * i.qty, 0); };
window.getCartCount = function() { return cartItems.reduce((s,i) => s + i.qty, 0); };
  ...
  if (cartItems.length === 0) {
    container.innerHTML = `
      <div class="cart-empty">
        ...
        <a href="shop.html" class="btn btn-outline btn-sm" style="margin-top:14px;" onclick="closeCart()">Browse Products</a>
      </div>`;
    return;
  }
  container.innerHTML = cartItems.map(item => `
    <div class="cart-item">
      <img src="${item.image}" alt="${item.name}" class="cart-item-img"
        onerror="this.src='https://images.unsplash.com/photo-1579952363873-27f3bade9f55?w=200&q=60'" />
...
function buildCartSidebar() {
  ...
  overlay.id = "cart-overlay"; overlay.onclick = closeCart;
  ...
  sidebar.innerHTML = `
    ...
    <div class="cart-footer">
      <div class="cart-upi-note">📱 Pay via UPI after placing order</div>
      <div class="cart-subtotal">
        <span class="cart-subtotal-label">Subtotal</span>
        <span class="cart-subtotal-val" id="cart-subtotal-val">₹0</span>
      </div>
      <a href="checkout.html" class="btn btn-green btn-full" style="margin-bottom:8px;">Proceed to Checkout →</a>
